...
export const proceduralEarthFragmentShader = `
  ...
  void main(){
    ...
    vec3 ocean=mix(vec3(.002,.004,.007),vec3(.014,.021,.030),smoothstep(.48,.64,mask)+veg*.16);
...
export const earthFragmentShader = `
  ...
  void main(){
    ...
    vec3 blackOcean=vec3(0.002,0.004,0.007);
    day=mix(day,blackOcean,oceanMask*0.92);
    vec3 night=pow(texture2D(nightMap,vUv).rgb,vec3(2.2));
    float sunDot=dot(normal,sunDir);
    float lit=clamp(sunDot,0.0,1.0);
    float dayAmount=smoothstep(-0.05,0.25,sunDot);
    vec3 dayLit=day*(0.55+0.75*lit);
    vec3 nightGlow=night*0.5*(1.0-step(0.5,dayAmount));
    vec3 color=mix(nightGlow,dayLit,dayAmount);
    color+=day*0.06;
    vec3 halfDir=normalize(sunDir+viewDir);
    float spec=pow(max(dot(normal,halfDir),0.0),64.0)*oceanMask*dayAmount;
    color+=vec3(0.88,0.94,1.0)*spec*0.22;
    float fres=pow(1.0-max(dot(normal,viewDir),0.0),2.5);
    color+=vec3(0.30,0.55,1.0)*fres*dayAmount*0.6;
